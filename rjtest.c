...
static void print_block(rijn_block_t *b)
{
	//printf("print block : \n");
		for (int j = 0; j < 4 ; j++)
		{
				   ...
				   unsigned char *str)
{
    ...
    while (*str)
    {
        ...
        if (DEBUG) printf("\nconcat encypted1 : ");
        hex_dump((const unsigned char*)ciphertext, 17, stderr);fflush (stdout);
		exit (1);
        if (concat == NULL)
        {
...
//  concat[16*(h) +1 ] = '\0';
    if (DEBUG) printf("\nconcat encypted2 : ");
    hex_dump((const unsigned char*)concat, 17, stdout);fflush (stdout);
//  sleep(3);
    return (concat);
}
...
int main(int ac, char **av)
{
    ...
    jj = rijn_init();
    rebuild = rijn_build_decrypt(&jj, (unsigned char *)concat);


    hex_dump((const unsigned char*)concat, 128, stderr);
//  if (DEBUG)
    printf("\nrebuild string = %s\n\n", rebuild );

    hex_dump((const unsigned char*)rebuild, 128, stderr);
    return 0;
}
*/
...
char *ft_strdup(const char *s1)
{
	...
	if (!s1)
		return (NULL);
	i = 0;
	t = (char *)malloc(strlen(s1) * sizeof(char *));
	if (t == NULL)
		return (t);
	while (s1[i] != '\0')
	{
...
void  test(void)
{
	...
	char *sa = "AAAABBBBCCCCDDDDEEEEFFFFGGGGHHHHaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";

    strncpy((char *)plaintext, sa, 32);
	rijn_encrypt(&sched, ciphertext, plaintext);
	printf("\ncipher sa = ");
...
char **stock_crypt(char *str, char **base)
{
	char **tab;
	int i =0;
	if (base == NULL)
	{
		...
		tab[0] = malloc (sizeof(char) * 32) ;
		strncpy(tab[0], str, 32);
		tab[1] = NULL;
		return tab;
	...
	else
	{
		...
		while (base[i])
		{
			tab[i] = malloc (sizeof(char) * 32) ;
			strncpy(tab[i], base[i],32);
			i++;
		}
		tab[i] = NULL;
		return tab;
...
//#endif*/
    ...
    bzero(str, 300);
    str = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbccccccccccccccccccccccccccccccccccccccdddddd\
dddddddddddddddddddddddd";
    rijn_keysched_t sched = { 0 };
    rijn_keysched_t sched1 = { 0 };
    unsigned char plaintext[32] = {};
    unsigned char ciphertext[32] = {};
    unsigned char iv[32] = {};
    int i = 0;
...
//        printf("%s\n",plaintext);
        ...
        putchar('\n');
//printf("\n-----\nplaintext = %s,\n iv = %s\n----------",plaintext, iv);
//rijn_sched_key(&sched1, &key, &param);
        rijn_decrypt(&sched, plaintext, ciphertext);
...
//	test();
//	return 0;
    str = "AAAABBBBCCCCDDDDEEEEFFFFGGGGHHHHaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaabbbbbbbbbbbbbb\0";
	char *str2 = ft_strdup("AAAABBBBCCCCDDDDEEEEFFFFGGGGHHHHaaaaaaaaaaaaa");
...
//plaintext[0] = 'a';
        ...
        //printf("plaintext = %s, iv = %s\n",plaintext, iv);
        rijn_encrypt(&sched, ciphertext, plaintext);
		tab = stock_crypt((char*)ciphertext, tab);
        printf("\n plaintx  = %s--\n",plaintext);
        hex_dump(plaintext, 32, stdout);
		putchar('\n');
        printf("\n crypty  = %s--\n",ciphertext);
        hex_dump(ciphertext, 32, stdout);
		putchar('\n');
        putchar('_');
//		stock_crypt(ciphertext);
//printf("\n-----\nplaintext = %s,\n iv = %s\n----------",plaintext, iv);
...
rijn_sched_key(&sched, &key, &param);
		...
		while (tab[i])
		{
			hex_dump(tab[i], 32, stdout);
			printf("\n");

			strncpy(ciphertext, tab[i], 32);
			hex_dump(tab[i], 32, stdout);
			printf("\n");
